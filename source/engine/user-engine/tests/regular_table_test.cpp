...
TEST_F(RegularTableTest, CollectGarbage) {
  ...
  for (auto&& entry : tt_) {
    entry.Init(0x334, HAND_ZERO);
    entry.UpdateUnknown(0, 3, 3, i++, 334, HAND_ZERO);
  }
...
TEST_F(RegularTableTest, SaveLoad) {
  ...
  p1->UpdateUnknown(334, 1, 1, komori::tt::detail::kTTSaveAmountThreshold + 1, 0x334, HAND_ZERO);
  auto p2 = tt_.PointerOf(board_key2);
  p2->Init(board_key2, hand2);
  p2->UpdateUnknown(334, 1, 1, komori::tt::detail::kTTSaveAmountThreshold - 1, 0x334, HAND_ZERO);
  ASSERT_NE(&*p1, &*p2);
